...
with st.expander("🧭 Market Indicator at a Glance", expanded=False):
    ...
    def draw_indicator_bar(label, value, colors, marker_label=""):
        fig = go.Figure()
        last = 0
        for color, end in colors:
            fig.add_shape(type="rect", x0=last, x1=end, y0=0, y1=1,
                          fillcolor=color, opacity=0.3, line_width=0)
            last = end
        fig.add_shape(type="line", x0=value, x1=value, y0=0, y1=1, line=dict(color="black", width=4))
        fig.update_layout(height=40, margin=dict(l=10, r=10, t=10, b=10),
                          xaxis=dict(range=[0, 100], visible=False),
                          yaxis=dict(visible=False),
                          plot_bgcolor="white", paper_bgcolor="rgba(0,0,0,0)")
        st.markdown(f"**{label}:** {marker_label or str(value)}")
        st.plotly_chart(fig, use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        draw_indicator_bar("BTC 1h Change", 52, [("red", 33), ("yellow", 66), ("green", 100)], "0.02%")
    ...
    with col2:
        ...
        fig.add_shape(type="line", x0=100, x1=100, y0=0, y1=1, line=dict(color="black", width=4))  # Bullish marker
        fig.update_layout(height=40, margin=dict(l=10, r=10, t=10, b=10),
                          xaxis=dict(range=[0, 100], visible=False),
                          yaxis=dict(visible=False),
                          plot_bgcolor="white", paper_bgcolor="rgba(0,0,0,0)")
        ...
        draw_indicator_bar("EMA Trend", 75, [("red", 33), ("yellow", 66), ("green", 100)], "Above 50 EMA")
        draw_indicator_bar("Fear & Greed", 53, [("red", 33), ("yellow", 66), ("green", 100)], "53 (Neutral)")
